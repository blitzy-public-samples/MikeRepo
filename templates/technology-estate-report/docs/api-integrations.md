...
| Endpoint | Purpose | Consumed By |
|---|---|---|
| `GET /products/{product}` | List of releases for a product (e.g., `/products/python`, `/products/nodejs`, `/products/postgresql`) | Maturity |
| `GET /products/{product}/releases/{cycle}` | Specific release-cycle data (e.g., `/products/python/releases/3.10`) | Maturity |

The `{product}` path segment is the endoflife.date product slug (lowercase, hyphenated where applicable). The product
slug resolution table — mapping a detected runtime or library to its endoflife.date slug — is documented in
[`./facets.md`](./facets.md) § Maturity Summary § Product Slug Resolution.

### 4.9 Response Format

The response is JSON. For a `GET /products/{product}` request, the body is an array of release entries; each entry
contains the following keys:

- **`cycle`** — the release-cycle identifier (e.g., `"3.10"` for Python 3.10, `"18"` for Node.js 18, `"15"` for
  PostgreSQL 15)
- **`eol`** — boolean `false` when the cycle is currently supported, OR an ISO 8601 date string (e.g.,
  `"2025-12-31"`) when the cycle has a published EOL date. The Maturity facet checks: if `false`, the runtime is
  supported; if a date, the runtime is past EOL when the date is in the past relative to the run date.
- **`latest`** — the latest patch-level release within the cycle (e.g., `"3.10.13"` for Python 3.10)
- **`releaseDate`** — ISO 8601 date of the cycle's initial release
- **`lts`** — boolean indicating long-term support designation (where applicable)

A representative response excerpt:

```json
[
  { "cycle": "3.12", "eol": false, "latest": "3.12.7", "releaseDate": "2023-10-02", "lts": false },
  { "cycle": "3.11", "eol": "2027-10-31", "latest": "3.11.10", "releaseDate": "2022-10-24", "lts": false },
  { "cycle": "3.10", "eol": "2026-10-31", "latest": "3.10.15", "releaseDate": "2021-10-04", "lts": false },
  { "cycle": "3.9", "eol": "2025-10-31", "latest": "3.9.20", "releaseDate": "2020-10-05", "lts": false },
  { "cycle": "3.8", "eol": "2024-10-07", "latest": "3.8.20", "releaseDate": "2019-10-14", "lts": false }
]
```
...
### 11.1 Canonical Sources Referenced
...
- [`../config/facets.yaml`](../config/facets.yaml) — machine-readable version pins for `apis.*` and the
  `severity_tier_mapping`, `apis_*`, and `facets.security.cve_lookup.*` configuration consumed by the CVE Scanner
...
- [`../CHANGELOG.md`](../CHANGELOG.md) — versioned record of API version-pin updates per § 10.3

### 11.2 Sibling Documentation Pages Referenced

- [`./facets.md`](./facets.md) — Security Summary facet's CVE pipeline that consumes NVD (§ 5) and OSV (§ 6); Maturity
  Summary facet's EOL pipeline that consumes endoflife.date (§ 4); Tech Stack and Complexity facets' artifact source
...
### 11.3 No Outbound Links Beyond the Package
...
`https://api.github.com`, `https://services.nvd.nist.gov`) appear exclusively as **literal text in code fences or
prose** for documentation purposes; they are NOT clickable Markdown links and they MUST NOT appear elsewhere in the
package as primary content.
