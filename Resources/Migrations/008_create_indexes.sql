...
-- Partial name match using LIKE 'prefix%' leverages B-tree prefix scanning.
CREATE INDEX idx_accounts_name ON accounts (account_name);

-- Fund type filtering: open_mutual_fund, closed_mutual_fund, etf, hedge_fund, sma, uma.
CREATE INDEX idx_accounts_fund_type ON accounts (fund_type);

-- Account group filtering and entitlement-scoped queries.
...
-- Per-account position retrieval for NAV valuation and UI display.
-- Complements FK fk_positions_account from migration 006.
CREATE INDEX idx_positions_account ON positions (account_id);

-- Instrument-based lookups for cross-account position aggregation.
...
-- Per-account transaction history retrieval.
-- Complements FK fk_transactions_account from migration 007.
CREATE INDEX idx_transactions_account ON transactions (account_id);

-- Time-range queries on the immutable transaction log.
...
-- Single-column ticker index for ticker-only lookups.
CREATE INDEX idx_reference_data_ticker ON reference_data (ticker);

-- Single-column date index for date-range queries.
CREATE INDEX idx_reference_data_date ON reference_data (market_date);

-- Composite covering index for the most common valuation query pattern:
-- SELECT eod_bid, eod_ask FROM reference_data WHERE ticker = ? AND market_date = ?
...
-- Per-user entitlement lookups (e.g., "what groups can this user access?").
...
-- Per-group entitlement lookups (e.g., "which users have access to this group?").
